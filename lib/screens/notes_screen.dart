      ...
      builder: (ctx) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('卡片颜色', style: Theme.of(ctx).textTheme.titleMedium),
      ...
      builder: (ctx) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                '为${selectedNoteIds.length}条笔记设置颜色',
    ...
    if (mounted) {
      if (success) {
        final folderName = selectedFolderId.isEmpty
            ? '无文件夹'
            : folderRepo.getNoteFolderById(selectedFolderId)?.name ?? '未知';

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('笔记已移至 $folderName'),
            backgroundColor: Colors.green,
          ),
        );
      } else {
