...
class _EventEditorScreenState extends ConsumerState<EventEditorScreen> {
    ...
    return Column(
      ...
      children: [
        ...
        _buildQuickActionChip(
          ...
          label: '优先级：${_priorityLabel(_priority)}',
  ...
  String _getStartDateLabel() {
    ...
    return '开始：${DateFormatters.formatSmart(_startDate!, now: now, includeTime: false)}';
  }
  ...
  String _getEndDateLabel() {
    ...
    return '结束：${DateFormatters.formatSmart(_endDate!, now: now, includeTime: false)}';
  }

  String _getStartTimeLabel() {
    if (_startTime == null) return '设置开始时间';
    final prefs = ref.read(preferencesStateProvider);
    final use24Hour = prefs.resolveUse24Hour(
      MediaQuery.of(context).alwaysUse24HourFormat,
    );
    return '开始：${DateFormatters.formatTimeOfDay(_startTime!.hour, _startTime!.minute, use24Hour: use24Hour)}';
  }

  String _getEndTimeLabel() {
    if (_endTime == null) return '设置结束时间';
    final prefs = ref.read(preferencesStateProvider);
    final use24Hour = prefs.resolveUse24Hour(
      MediaQuery.of(context).alwaysUse24HourFormat,
    );
    return '结束：${DateFormatters.formatTimeOfDay(_endTime!.hour, _endTime!.minute, use24Hour: use24Hour)}';
  }

  String _getReminderLabel() {
    if (_reminderOffsetsMinutes.isEmpty) return '添加提醒';
    if (_reminderOffsetsMinutes.length == 1) return '已设置1个提醒';
    return '已设置${_reminderOffsetsMinutes.length}个提醒';
  }

  String _getRepeatLabel() {
    switch (_repeatType) {
      case 'daily':
        if (_repeatInterval == 1) return '每天重复';
        return '每$_repeatInterval天重复';
      case 'weekly':
        if (_repeatInterval == 1) return '每周重复';
        return '每$_repeatInterval周重复';
      case 'monthly':
        if (_repeatInterval == 1) return '每月重复';
        return '每$_repeatInterval月重复';
      case 'yearly':
        if (_repeatInterval == 1) return '每年重复';
        return '每$_repeatInterval年重复';
      case 'custom':
        return '自定义重复';
      default:
        return '不重复';
    ...
    return Consumer(
      builder: (context, ref, child) {
        ...
        return foldersAsync.when(
          ...
          error: (error, stack) => Text('加载文件夹出错：$error'),
        );
      },
    );
  }
  ...
  /// Converts priority value to Chinese display label
                      ...
                      if (_repeatType != 'none') ...[
                        ...
                        ListTile(
                          ...
                          title: Text(
                            _repeatEndDate == null
                                ? '无结束日期'
                                : '结束于${DateFormatters.formatSmart(_repeatEndDate!, now: ref.read(clockProvider).now(), includeTime: false)}',
                          ),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('保存事件出错：$e')));
      }
    } finally {
      if (mounted) {
