...
class _TemplateManagementScreenState
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('文件夹模板')),
          ...
          return ListView(
            children: [
              ...
              if (builtInTemplates.isNotEmpty) ...[
                _buildSectionHeader(context, '内置模板'),
              ...
              if (customTemplates.isNotEmpty) ...[
                _buildSectionHeader(context, '自定义模板'),
                ...customTemplates.map(
                  (template) =>
                      _buildTemplateListTile(context, template, colorScheme),
                ),
        ...
        children: [
          ...
          if (isCustomized)
            Padding(
              ...
              child: Chip(label: const Text('已修改')),
            ),
        ],
      ),
      subtitle: Text(
        template.description ?? '${template.taskTemplates.length}个任务',
        maxLines: 1,
      ...
      builder: (context) => TemplateEditorDialog(
        onSave: (template) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('模板"${template.name}"已创建')),
          );
        },
      ),
    );
    ...
    showDialog(
      ...
      builder: (context) => TemplateEditorDialog(
        ...
        onSave: (updatedTemplate) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('模板"${updatedTemplate.name}"已更新'),
            ),
          );
        },
      ),
  ...
  void _duplicateTemplate(FolderTemplate template) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('模板"${duplicatedTemplate.name}"已创建')),
    );
  }
