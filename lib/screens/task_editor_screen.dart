...
class _TaskEditorScreenState extends ConsumerState<TaskEditorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.todo == null ? '新建任务' : '编辑任务'),
  ...
  String _getDueDateLabel() {
    ...
    if (_dueDate == null) {
      ...
      return '截止：${DateFormatters.formatSmart(_dueDate!, now: now, includeTime: false)}';
    } else {
    ...
    switch (_repeatType) {
      ...
      case 'weekly':
        if (_repeatInterval == 1) {
          if (_repeatDays.isEmpty) return '每周重复';
          return '每周重复';
        }
  ...
  Future<void> _selectDuration() async {
    ...
    final durations = <int, String>{
      ...
      240: '4 小时',
      480: '8 小时',
      0: '自定义……', // Placeholder for custom selection
    };
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('保存任务出错：$e')));
      }
    } finally {
      if (mounted) {
