...
class OverviewTab extends ConsumerStatefulWidget {
  ...
  static const _sectionLabels = {
    'clock': '时钟',
    'greeting': '问候语',
    'folder_shortcuts': '文件夹快捷方式',
    'progress': '任务进度',
    'todos': '待办任务',
    'events': '即将开始的事件',
    'latest_notes': '最新笔记',
    'recent_settings': '快速设置',
  };
...
class _NameEditDialogState extends State<_NameEditDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: const Text('您的名字'),
      content: TextField(
        controller: _controller,
        autofocus: true,
        decoration: const InputDecoration(hintText: '输入您的名字'),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      '${stats.overdue > 0 ? ' · ${stats.overdue} 逾期' : ''}',
                      style: theme.textTheme.bodySmall?.copyWith(
                        color: colorScheme.onSurfaceVariant,
                      ),
                    ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(height: spacing.s8),
        if (events.isEmpty)
          _EmptyCard(icon: Icons.event_available, message: '暂无即将开始的事件')
        else
          ...events.map((event) {
    ...
    return Column(
      ...
      children: [
        ...
        notesAsync.when(
          data: (notes) {
            if (notes.isEmpty) {
              return _EmptyCard(icon: Icons.note_add, message: '暂无笔记');
            }
            return Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
          ...
          error: (_, _) =>
              _EmptyCard(icon: Icons.warning, message: '加载笔记出错'),
        ),
      ],
    );
  }
    ...
    return Column(
      ...
      children: [
        ...
        if (shortcuts.isEmpty)
          _EmptyCard(icon: Icons.folder_off_outlined, message: '暂无文件夹')
        else
          SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: Row(
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  Text(
                    folder.type == _FolderType.task ? '任务' : '笔记',
