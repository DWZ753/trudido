...
class _QuillNoteEditorScreenState extends ConsumerState<QuillNoteEditorScreen> {
  ...
  void _insertTable(int insertIndex) {
    ...
    showDialog<(int, int)>(
      ...
      final cells = List.generate(
        rows,
        (r) => List.generate(cols, (c) => r == 0 ? '列 ${c + 1}' : ''),
      );
  ...
  Future<File?> _showMediaPickerDialog(MediaType type) async {
    ...
    return showDialog<File?>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(type == MediaType.photo ? '添加图片' : '添加视频'),
        content: Column(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${type.capitalize()} 添加成功')),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('添加$type出错：$e')));
      }
    }
  }
  ...
  Future<void> _saveNoteInternal({bool showFeedback = true}) async {
    ...
    if (_quillController.document.toPlainText().trim().isEmpty &&
      ...
      if (showFeedback && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('请输入标题或内容'),
            backgroundColor: Theme.of(context).colorScheme.errorContainer,
          ),
        );
      }
      throw Exception('笔记不能为空');
    }
  ...
  IconData _getStatusIcon() {
    if (_saveStatus.contains('保存中')) {
      return Icons.sync;
    } else if (_saveStatus.contains('保存')) {
      return Icons.check_circle;
    } else if (_saveStatus.contains('失败')) {
      return Icons.error;
    }
    return Icons.info;
  }
  ...
  void _showNoteHistory() {
    ...
    showNoteHistoryBottomSheet(
      context: context,
      noteId: noteId,
      noteTitle: _originalNote?.title ?? '无标题',
  ...
  Future<void> _openLink(String url) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('打开链接出错')));
      }
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              ExpressiveFloatingActionButton(
                ...
                tooltip: _isReadMode ? '切换为编辑' : '切换为阅读',
                backgroundColor:
                    resolveNoteColor(
                      _originalNote?.colorValue,
                      Theme.of(context).brightness,
          ...
          actions: [
            ...
            if (!_useFloatingToolbar && !_isReadMode)
              ExpressiveIconButton(
                ...
                tooltip: _hideToolbar ? '显示格式' : '隐藏格式',
                onPressed: () {
                  setState(() {
