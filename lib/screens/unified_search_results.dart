...
class UnifiedSearchResults extends ConsumerWidget {
        ...
        return Column(
          ...
          children: [
            Padding(
              padding: const EdgeInsets.fromLTRB(16, 16, 16, 8),
              child: Text(
                '笔记文件夹 (${nonVaultNoteFolders.length})',
                style: Theme.of(
                  context,
                ).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600),
              ),
  ...
  Widget _buildScopeChips(
    ...
    const categories = [
      ('tasks', '任务', Icons.check_circle_outline),
      ('events', '事件', Icons.event_outlined),
      ('notes', '笔记', Icons.note_outlined),
      ('folders', '文件夹', Icons.folder_outlined),
  ...
  Widget _buildResultsSummary(
    ...
    final parts = <String>[];
    if (taskCount > 0) parts.add('$taskCount个任务');
    if (eventCount > 0) {
      parts.add('$eventCount个事件');
    }
    if (noteCount > 0) parts.add('$noteCount个笔记');
    final totalFolders = folderCount + noteFolderCount;
    if (totalFolders > 0) {
      parts.add('$totalFolders个文件夹');
    }
    if (settingCount > 0) {
      parts.add('$settingCount个设置');
    }
        ...
        if (capped)
          Padding(
            ...
            child: TextButton(
              ...
              child: Text('显示另外${total - widget.maxVisible}个'),
            ),
          ),
      ],
    );
