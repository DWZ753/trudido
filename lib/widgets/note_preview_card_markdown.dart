...
class NotePreviewCard extends ConsumerWidget {
          ...
          final confirmed =
              await showDialog<bool>(
                ...
                builder: (context) => AlertDialog(
                  title: const Text('移动到回收站'),
                  content: const Text(
                    '将此笔记移动到回收站？之后可以从回收站恢复。',
                  ),
                  actions: [
  ...
  String _formatCompactDate(DateTime date, bool use24Hour) {
    ...
    if (dateDay == today) {
      return '今天 ${DateFormatters.formatTime(date, use24Hour: use24Hour)}';
    } else if (dateDay == yesterday) {
      return '昨天 ${DateFormatters.formatTime(date, use24Hour: use24Hour)}';
