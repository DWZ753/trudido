...
class TodoListTab extends ConsumerWidget {
  ...
  Widget _buildEmptyState(
    ...
    final itemLabel = isEventsOnly ? '事件' : (isAll ? '项目' : '任务');
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            spacing.gapV16,
            Text(
              isSearching ? '未找到$itemLabel' : '暂无$itemLabel',
            ...
            Text(
              isSearching
                  ...
                  : isAll
                  ? '点击 + 按钮添加任务或事件'
                  : '点击 + 按钮添加您的第一个${isEventsOnly ? '事件' : '任务'}',
      ...
      children: [
        ...
        if (hasScheduledSection) ...[
          ...
          if (scheduledTomorrow.isNotEmpty) ...[
            _buildSubHeader(context, ref, '明天'),
          ...
          if (scheduledThisWeek.isNotEmpty) ...[
            _buildSubHeader(context, ref, '本周'),
          ...
          if (scheduledLater.isNotEmpty) ...[
            _buildSubHeader(context, ref, '稍后'),
            ...scheduledLater.map(
              (item) => _buildListItem(context, ref, item, theme, colorScheme),
            ),
          ],
      ...
      confirmDismiss: (direction) async {
        ...
        if (action == 'delete') {
          final confirmed = await showDialog<bool>(
            ...
            builder: (context) => AlertDialog(
              title: const Text('移至回收站'),
              content: Text('将"${holiday.name}"移至回收站？'),
              actions: [
                ExpressiveTextButton(
      ...
      confirmDismiss: (direction) async {
        ...
        if (action == 'delete') {
          final confirmed = await showDialog<bool>(
            ...
            builder: (context) => AlertDialog(
              title: const Text('移至回收站'),
              content: Text('将"${event.text}"移至回收站？'),
              actions: [
                ExpressiveTextButton(
                  onPressed: () => Navigator.pop(context, false),
                  child: const Text('取消'),
